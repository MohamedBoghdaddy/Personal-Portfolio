  ...
  return (
    <main className="page-shell">
      <SectionHeader
        ...
        title="Early Builds Archive"
        description="Cold storage for older builds, experiments, foundational logic tests, and legacy prototypes."
        icon="folder_zip"
      />
        ...
        <section className="md:col-span-8">
          <div className="grid grid-cols-1 gap-sm sm:grid-cols-2">
            {archiveItems.map((item) => (
              <GlassCard key={item.title} className="p-md" accent="secondary">
        ...
        <aside className="space-y-md md:col-span-4">
          <GlassCard className="p-md">
            ...
            <div className="space-y-sm">
              {[
                ["Total Artifacts", "Archived set"],
                ["Latest Entry", "Recent"],
                ["Status", "Cold storage"]
              ].map(([label, value]) => (
                <div key={label} className="flex justify-between gap-sm border-b border-outline-variant/10 pb-xs font-mono text-sm">
          ...
          <VisualPlaceholder icon="folder_zip" title="Repository Archive TODO" accent="primary" className="aspect-square" />
        </aside>
      </div>

      <section className="mt-xl border-t border-outline-variant/30 py-lg">
        <div className="mb-md flex flex-col justify-between gap-sm md:flex-row md:items-center">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-outline">Dependency notes: qualitative</span>
        </div>
        <div className="flex flex-wrap gap-sm opacity-70 grayscale transition hover:opacity-100 hover:grayscale-0">
